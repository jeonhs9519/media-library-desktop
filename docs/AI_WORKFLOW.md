...
## 11. Tauri Migration Rule

- Tauri 전환 요청이 있어도 기존 Electron 프로젝트를 한 번에 변환하지 않습니다.
- 먼저 기존 앱의 기능, 화면, DB, IPC/API, 파일 처리 흐름을 분석해 문서화합니다.
- 이후 Tauri + Rust + Svelte 신규 프로젝트에서 기능 단위로 재구축합니다.
- 기존 앱은 재구축 완료 전까지 기준 구현체로 유지합니다.
