...
- 상세 정보 모달의 태그 입력은 공용 `TagSearchInput` 기반이며, 현재 프로필에서 실제 사용 중인 태그만 검색 후보로 표시하고 위/아래 방향키 선택 후 Enter로 등록할 수 있습니다. 옵션 목록은 입력란의 active descendant로만 관리하고, Tab/Shift+Tab은 목록 항목이 아니라 입력란 주변 컨트롤로 이동합니다.
...
- 라이브러리 썸네일 목록은 카드 크기를 유지한 채 grid 칼럼을 `space-between` 방식으로 분배하며, 마지막 줄도 첫 줄과 같은 칼럼 간격으로 왼쪽부터 배치합니다.
...
- 라이브러리 목록 컨텍스트 메뉴 또는 drag and drop으로 플레이리스트에 항목을 추가하면 플레이리스트 목록으로 포커스를 이동하고, 추가된 항목을 선택 상태로 맞춥니다. 상세보기 팝업의 `리스트에 추가`는 기존 포커스를 유지합니다.
...
- 플레이리스트 항목 목록은 Tab 키로 목록에 진입한 뒤 위/아래 방향키로 항목을 선택할 수 있으며, `Delete` 키로 선택 항목을 제거할 수 있습니다.
- 플레이리스트 항목을 `Delete` 키 또는 제거 버튼으로 삭제한 뒤에는 삭제 처리가 끝난 후 포커스가 플레이리스트 목록 컨테이너로 돌아가며, 곧바로 방향키 탐색을 이어갈 수 있습니다.
- 플레이리스트 항목 내부 제거 버튼은 Tab 순서와 라이브러리 목록의 Shift+Tab 이전 포커스 계산에서 제외하고, 항목 포커스 상태에서 Tab/Shift+Tab을 누르면 목록 밖의 다음/이전 포커스 대상으로 이동합니다.
- 플레이리스트 항목 컨텍스트 메뉴는 `여기서부터 재생`, `위로 한 칸 이동`, `아래로 한 칸 이동`, `상세정보 확인`, `목록에서 제거`를 제공하며 `Enter / Space`, `Ctrl + ↑`, `Ctrl + ↓`, `Alt + Enter`, `Delete` 단축키 표기를 함께 표시합니다.
- 공용 컨텍스트 메뉴는 메뉴 바깥 클릭과 전역 `Escape`로 닫히며, 라이브러리 카드 메뉴는 `Escape`로 닫을 때 현재 active 카드로 포커스를 돌립니다. 플레이리스트 항목 메뉴는 `Escape` 또는 플레이리스트 패널 내부 클릭으로 닫힐 때만 포커스가 플레이리스트 목록으로 돌아갑니다.
- 공용 컨텍스트 메뉴의 하위 메뉴는 호출한 상위 항목 위치에 맞춰 표시하고, 오른쪽 공간이 부족하면 왼쪽으로 열어 상위 메뉴를 덮지 않도록 합니다.
- 상세보기 팝업은 라이브러리 목록과 플레이리스트 중 호출 출처를 구분해 닫힌 뒤 해당 목록으로 포커스를 돌립니다.
...
- 동영상 뷰어 컨텍스트 메뉴는 재생/반복 재생, 썸네일 설정/전체화면, 플레이리스트 항목, 파일 위치 열기/뷰어 종료 순서로 표시합니다.
...
- 체크박스와 라디오 버튼은 공용 `ChoiceInput` 컴포넌트와 동일한 custom input 스타일을 사용합니다.
- select UI는 공용 `Dropdown` 컴포넌트로 대체했으며, Tab 포커스 이동과 위/아래 방향키 option 이동을 지원합니다.
...
- 상세 정보 모달 상단의 `리스트에 추가`, `외부에서 열기`는 정사각형 아이콘 버튼으로 표시합니다.
...
- 상세 정보 모달을 닫으면 호출 출처에 따라 라이브러리 썸네일 목록 또는 플레이리스트 목록으로 포커스가 돌아갑니다.
...
- 현재 브랜치는 `main`입니다.
- `v0.1.0` 첫 사용 가능 배포판 릴리즈를 완료했습니다.
- 이후 주요 작업은 테스트 보강, 문서 정리, Tauri 재구축 준비 문서화입니다.
